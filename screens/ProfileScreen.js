...
const ProfileScreen = ({ route, navigation }) => {
    ...
    const fetchUserData = async () => {
        try {
            ...
            const response = await fetch(`https://smart-farming-api.onrender.com/api/v1/user/userbyid/${id}`);
    ...
    const handleDeleteAccount = async () => {
        try {
            const id = route.params.userId; // Get the user ID from the route parameters
            const response = await fetch(`https://smart-farming-api.onrender.com/api/v1/user/deleteuser/${id}`, {
                method: 'DELETE',
            });
